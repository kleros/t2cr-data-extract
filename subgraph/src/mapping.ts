...
export function handleRequestSubmitted(event: RequestSubmitted): void {
  ...
  let id =
    ...
    "-" +
    token.numberOfRequests.minus(BigInt.fromI32(1)).toString();
  let request = new Request(id);

  request.timestamp = event.block.timestamp;
  request.result = PENDING;
  request.type = event.params._registrationRequest ? REGISTRATION : REMOVAL;
...
export function handleTokenStatusChange(event: TokenStatusChange): void {
  ...
  let token = Token.load(event.params._tokenID.toHexString());
  if (token == null) {
    log.error("T2CR: token {} not found. Bailing handleTokenStatusChange.", [
      event.params._tokenID.toHexString(),
    ]);
    return;
  }
  ...
  if (request == null) {
    log.error("T2CR: Request {} not found. Bailing handleTokenStatusChange.", [
      id,
    ]);
    return;
  }
